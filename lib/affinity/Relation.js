...
var Relation = Set.extend(
    {
        ...
        bindEvents: function () {
            ...
            this.ee.once('beforeGetElements', function () {
                ...
                if (constraintArguments) {
                    ...
                    if (constraintArguments.fk) {
                        _.forEach(constraintArguments.fk, function (fk) {
                            fk.relation = that;
                            that.fks().add(new ForeignKey(fk));
                        })
                    }
                ...
                constraints = that.uks().clone();

                if (that.pk() !== null) {
                    constraints.add(that.pk());
                }
                ...
                constraints.each(
                        ...
                        for (var a = index + 1, len = constraints.length(); a < len; a++) {
                            ...
                            var checkedKeyHeader = constraints.atIndex(a).header;
         ...
         */
        pk : function(primaryKey){

            if (primaryKey){
                this._pk = new Key(this, primaryKey);
            } else {
                return this._pk;
            }
