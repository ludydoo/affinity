...

    {
        ...
        bindEvents: function () {
            ...
            this.ee.once('beforeGetBody', function () {
                ...
                left.each(function (tuple) { that.afterAdd(left, tuple); });

                right.each(function (tuple) { that.afterAdd(right, tuple);});

            });

            left.ee.on('afterAdd', function(tuple){

                that.afterAdd(left, tuple);

            });

            right.ee.on('afterAdd', function(tuple){

                that.afterAdd(right, tuple);

            });

            left.ee.on('afterRemove', function(tuple, index){

                that.afterRemove(left, tuple);

            });

            right.ee.on('afterRemove', function(tuple, index){

                that.afterRemove(right, tuple);
         ...
         */
        afterAdd : function(relation, tuple1){
         ...
         */
        afterRemove : function(relation, removedTuple){
         ...
         */
        afterUpdate : function(relation, tuple, attributeName, value){
            ...
            if (that.commonAttributes.get(attributeName) !== null){
            // The updated attribute is part of the common attributes



            } else{
            // The updated attribute is not part of the common attributes
