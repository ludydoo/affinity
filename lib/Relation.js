...
var Relation = Set.extend(
     ...
     */
    {
         ...
         *
         *
         * //To construct a relation, arguments can either be passed in "raw" format :
         *
         * new affinity.Relation([{attr1 : Type}, {attr2 : Type}, {attr3 : Type}, ...],
         *                 [[val1, val2, val3],
         *                  [val1, val2, val3],
         *                  [val1, val2, val3],
         *                  [val1, val2, val3]]);
         *
         * //Note that val1 is mapped to attr1, val2 is mapped to attr2 and so on.
         * //When constructing a Relation this way, values are assigned by order they
         * //were declared in the header.
         *
         * //A relation can also be declared like this :
         *
         * new affinity.Relation(new affinity.Header({a : Type, b : Type, c : Type}),
        ...
        constructor: function (header, body) {
            ...
            if (header) {
                ...
                if (header instanceof Header) {
                    ...
                    // Header is already a Header object

                    this.header(header);

                } else if (_.isArray(header)) {

                    // Header is in raw format
                    ...
                    for (a = 0, headerLength = header.length; a < headerLength; a++) {

                        attributeName = Object.keys(header[a])[0];
                        attributeNames.push(attributeName);

                        attributeArgs = _.extend(args, header[a]);
                    }

                    this.header(new Header(attributeArgs));

                } else {

                    throw new TypeError('Wrong type for header argument. Expecting an array of objects or a type Header object.');
            ...
            if (body) {
                ...
                if (_.isArray(body)) {
                    ...
                    // Tuples passed in array format

                    for (b = 0, tupleCount = body.length; b < tupleCount; b++) {

                        tuple = body[b];

                        if (tuple instanceof Tuple) {

                            /// Tuple passed in object format

                            this.add(tuple)

                        } else if (_.isArray(tuple)) {

                            /// Tuple passed in raw format

                            tupleLength = tuple.length;

                            // Check if the header was passed in raw format too
                            if (rawFormat === false) {
                                throw new SyntaxError('Must pass header in raw format to pass body in raw format');
                            }

                            args = {};

                            // Creating a hash containing the tuple arguments
                            // First arg is mapped to the first header attribute
                            for (a = 0; a < tupleLength; a++) {
                                args[attributeNames[a]] = tuple[a];
                            }

                            this.add(new Tuple(args));
                    ...
                    throw new TypeError('Tuple argument of wrong type. Expected an array');
        ...
        print: function () {
            ...
            console.log(this.getPrintString());
        },
        ...
        getPrintString: function () {
            ...
            this.header().each(function (attribute) {
                ...
                var maxWidth = attribute.toString().length;

                attributes.push(attribute);

                this.each(function (tuple) {

                    var tupleWidth = tuple.get(attributeName).toString().length;

                    if (tupleWidth > maxWidth) {
                        maxWidth = tupleWidth;
                    }

                });

                attributeWidths[attributeName] = maxWidth;
            ...
            for (a = 0; a < attributes.length; a++) {
                ...
                maxWidth = attributeWidths[attribute.name];

                if (a == 0)
                    result += '+';

                for (b = 0; b < maxWidth + 2 * hSpacing; b++) {
                    result += '-';
                }
            ...
            for (a = 0; a < attributes.length; a++) {
                ...
                attribute = attributes[a];
                width = attribute.toString().length;
                maxWidth = attributeWidths[attribute.name];

                if (a == 0)
                    result += '|';

                for (b = 0; b < hSpacing; b++) {
                    result += ' ';
                }
            ...
            this.each(function (tuple) {
                ...
                for (var k = 0; k < attributes.length; k++) {
                    ...
                    var mw = attributeWidths[attribute.name];
                    ...
                    var tupleString = tupleAttribute.toString();
                    var tupleLength = tupleString.length;

                    result += tupleString;

                    for (b = 0; b < mw - tupleLength + 2*hSpacing-1; b++) {
                        result += ' ';
                    }
